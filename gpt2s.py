...
class DataLoader:
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        data_root = os.path.join(os.getenv('TMPDIR'), 'edu_fineweb10B')
    ...
    def next_batch(self):
        ...
        if (self.current_position + (self.num_processes * B * T + 1)) > len(self.tokens):
...
# exception handling for CUDA out of memory
# I dont know how much memory is needed, i catch the error in the slurm script
# and then retry with a smaller batch size
try:
    ...
    # Can probably do 32 on 120GB on Vram (microbatch 2 = 5.650GB, --> microbatch 32 = 90.432 GB )
    import sys
    B = int(sys.argv[1]) # micro batch size, set when launching the script
    ...
    # create model
    model = GPT(GPTConfig(vocab_size=50304))  # nicer number for cuda kernels (divisible by up to 128 --> many powers of 2)
    model.to(device)
